...
import sys # [AJC 2021-07-05] added for parallelisation
import glob # [AJC 2021-07-05] added for parallel processing file handling
import dill # [AJC 2021-07-05] added for parallel processing file handling
...
def record_parameters(sysdata, excecution_parameters, start_time):
            ...
            sysdata.write("\tNMF_replicate: {}\n".format(excecution_parameters["NMF_replicate"])) # [AJC 2021-07-05] added new parameter
...
def sigProfilerExtractor(input_type,
    ...
    section: A string. Whether to run the NMF or NMF-combination section of SigProfilerExtractor. Options are "nmf" and "combine". [AJC 2021-07-05]
    ...
    stability: A float. The cutoff thresh-hold of the average stability. Solutions with average stabilities below this threshold will not be considered. Default is 0.8.

    min_stability: A float. The cutoff thresh-hold of the minimum stability. Solutions with minimum stabilities below this threshold will not be considered. Default is 0.2.

    combined_stability: A float. The cutoff thresh-hold of the combined stability (sum of average and minimum stability). Solutions with combined stabilities below this thresh-hold will not be considered. Default is 1.0.


    DECOMPOSITION:-

    de_novo_fit_penalty: Float, optional. Takes any positive float. Defines the weak (remove) thresh-hold cutoff to be assigned denovo signatures to a sample. Default is 0.02.

    nnls_add_penalty: Float, optional. Takes any positive float. Defines the strong (add) thresh-hold cutoff to be assigned COSMIC signatures to a sample. Default is 0.05.

    nnls_remove_penalty: Float, optional. Takes any positive float. Defines the weak (remove) thresh-hold cutoff to be assigned COSMIC signatures to a sample. Default is 0.01.

    initial_remove_penalty: Float, optional. Takes any positive float. Defines the initial weak (remove) thresh-hold cutoff to be COSMIC assigned signatures to a sample. Default is 0.05.
    ...
    # [AJC 2021-07-05] added to ensure that when NMF replicates are completed in parallel, only a single replicate is completed by each job
    if (section == "nmf"):
        nmf_replicates = 1
...
# =============================================================================
    ...
    for m in mtypes:
        ...
        for i in range(startProcess,endProcess+1):
            ...
            # [AJC 2021-07-05] start added section allowing NMF extractions to be completed separately
            if (section == "nmf"):
            ...
            if (section == "combine"):
                all_results=[]
                for pickle_file in glob.iglob("*pkl"):
                    results = dill.load(open(pickle_file, "rb"))
                    result = results[0]
                ...
                processes = sub.decipher_signatures_combine(excecution_parameters,
                    results=all_results,
                    genomes= genomes,
                    mut_context=m,
                    i = i)
